...
export const grid = new DataGrid("grid");
grid.setRowHeight(30);

//Defining and perzonalizing Columns
grid.addColumn("Nombre", 200, (args) => {
...
grid.addColumn("Nacionalidad", 150, (args) => {
    args.row.addChild(new Label(args.fieldId));
    const lbl = window.w.get(args.fieldId) as Label;
...
grid.addColumn("&nbsp;", 40, (args) => {
    ...
    btn.subscribe({
        event: "click",
        then: () => {
            console.log("Desea eliminar a " + args.data.name);
        },
    });
    btn.setVariant("text");
    btn.setColor("error");
